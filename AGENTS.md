...
- **Explaining Style:** Avoid overly dense technical jargon. When a new concept is introduced, define it using a relatable, real-world analogy. Always show code if you're trying to reference something.
- **Referencing Repo Content:** When pointing to code, workflow files, or config in this repository, show the relevant snippet directly in the conversation first, then include the file path/line reference for full context. Do not rely on file links alone when the snippet is short enough to quote.
